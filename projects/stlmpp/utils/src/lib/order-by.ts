...
export function orderBy<T, K extends keyof T>(
  ...
  if (!keyOrCommand) {
    ...
    const getter = keyOrCommand.includes('.') ? getDeep : value => value[keyOrCommand];
    return sort(values)[order](entity => getter(entity, keyOrCommand));
  } else if (isArray(keyOrCommand)) {
    const getter = keyOrCommand.some(key => key.includes('.')) ? getDeep : (value, key) => value[key];
    return sort(values)[order]((keyOrCommand as string[]).map(key => entity => getter(entity, key)));
