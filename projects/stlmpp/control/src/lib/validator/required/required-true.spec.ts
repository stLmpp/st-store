...
describe('required true validator', () => {
  ...
  it('should validate', () => {
    ...
    expect(validator.validate(createFakeControl<boolean | null | undefined>(false))).toBeTrue();
    expect(validator.validate(createFakeControl<boolean | null | undefined>(true))).toBeNull();
  });
});
